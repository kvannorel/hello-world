...
public class CustomSettings {
	...
	public static String getPLTUser() {
		if(Settings.get("PROXY").equals("false")) {
			//Postgres
			return Settings.get("CONNECTION_DEFINITION", "user");
		} else {
			//Oracle
			return Settings.get("PROXY_SCHEMA_NAME_LOOKUP", "plt"); 
			// No user name defined in CIRCOO configuration server. We use the schema name for the username, it should be the same.
		}
	}
	
	public static String getPLTPass() {
		if(Settings.get("PROXY").equals("false")) {
			//Postgres
			return Settings.get("CONNECTION_DEFINITION", "password");
		} else {
			//Oracle
			return Settings.get("PROXY_USERS", "platform_password");
		}
	}
	
	public static String getPLTSchema() {
		if(Settings.get("PROXY").equals("false")) {
			//Postgres
			return Settings.get("SCHEMA_NAME_LOOKUP", "plt");
		} else {
			//Oracle
			return Settings.get("PROXY_SCHEMA_NAME_LOOKUP", "plt");
		}
