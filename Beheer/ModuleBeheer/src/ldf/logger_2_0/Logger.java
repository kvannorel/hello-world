...
public class Logger implements TalendJob {
				...
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Logger.this, new Object[] { e,
	...
	public static class row1Struct implements
		...
		final static byte[] commonByteArrayLock_LDF_Logger = new byte[0];
		static byte[] commonByteArray_LDF_Logger = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_LDF_Logger.length) {
					if (length < 1024 && commonByteArray_LDF_Logger.length == 0) {
						commonByteArray_LDF_Logger = new byte[1024];
					} else {
						commonByteArray_LDF_Logger = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_LDF_Logger, 0, length);
				strReturn = new String(commonByteArray_LDF_Logger, 0, length,
						utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_LDF_Logger) {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				for (int i_tFixedFlowInput_1 = 0; i_tFixedFlowInput_1 < 1; i_tFixedFlowInput_1++) {
					...
					if (row1.jobname == null) {
						...
						pstmt_tPostgresqlOutput_1.setString(1, row1.jobname);
					}

					if (row1.lastrun != null) {
						pstmt_tPostgresqlOutput_1.setTimestamp(2,
								new java.sql.Timestamp(row1.lastrun.getTime()));
					} else {
					...
					if (row1.result == null) {
						...
						pstmt_tPostgresqlOutput_1.setBoolean(3, row1.result);
					}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Logger.class.getClassLoader()
					.getResourceAsStream(
							"ldf/logger_2_0/contexts/" + contextStr
									+ ".properties");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : Logger");
		}
...
/************************************************************************************************
 * 41048 characters generated by Talend Open Studio for Data Integration on the
