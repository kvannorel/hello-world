...
/**
 * Job: Logger Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.2.1.20190620_1446
 * @status 
 */
public class Logger implements TalendJob {
...
public Boolean getResult(){
	...
	private final String jobName = "Logger";
		...
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Logger.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
    final static byte[] commonByteArrayLock_BRK_Logger = new byte[0];
    static byte[] commonByteArray_BRK_Logger = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_BRK_Logger.length) {
				if(length < 1024 && commonByteArray_BRK_Logger.length == 0) {
   					commonByteArray_BRK_Logger = new byte[1024];
				} else {
   					commonByteArray_BRK_Logger = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_BRK_Logger, 0, length);
			strReturn = new String(commonByteArray_BRK_Logger, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_BRK_Logger) {
...
if(dbschema_tPostgresqlOutput_1 == null || dbschema_tPostgresqlOutput_1.trim().length() == 0) {
	tableName_tPostgresqlOutput_1 = "job_results_ldf";
} else {
	tableName_tPostgresqlOutput_1 = dbschema_tPostgresqlOutput_1 + "\".\"" + "job_results_ldf";
}
 ...
 */
    ...
    public static void main(String[] args){
        final Logger LoggerClass = new Logger();

        int exitCode = LoggerClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Logger.class.getClassLoader().getResourceAsStream("brk/logger_2_0/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Logger.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Logger");
        }
...
/************************************************************************************************
 *     51856 characters generated by Talend Open Studio for Data Integration 
