...
/**
 * Job: JobLogger Purpose: <br>
 * Description: logs status of jobs into the ldf.job_results <br>
 * @author user@talend.com
 * @version 7.2.1.20190620_1446
 * @status 
 */
public class JobLogger implements TalendJob {
...
public Boolean getResult(){
	...
	private final String jobName = "JobLogger";
		...
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(JobLogger.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
    final static byte[] commonByteArrayLock_BRK_JobLogger = new byte[0];
    static byte[] commonByteArray_BRK_JobLogger = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_BRK_JobLogger.length) {
				if(length < 1024 && commonByteArray_BRK_JobLogger.length == 0) {
   					commonByteArray_BRK_JobLogger = new byte[1024];
				} else {
   					commonByteArray_BRK_JobLogger = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_BRK_JobLogger, 0, length);
			strReturn = new String(commonByteArray_BRK_JobLogger, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_BRK_JobLogger) {
...
if(dbschema_tPostgresqlOutput_1 == null || dbschema_tPostgresqlOutput_1.trim().length() == 0) {
	tableName_tPostgresqlOutput_1 = "job_results_brk";
} else {
	tableName_tPostgresqlOutput_1 = dbschema_tPostgresqlOutput_1 + "\".\"" + "job_results_brk";
}
...
int count_tPostgresqlOutput_1=0;
	    String insert_tPostgresqlOutput_1 = "INSERT INTO \"" + tableName_tPostgresqlOutput_1 + "\" (\"jobname\",\"lastrun\",\"result\") VALUES (?,?,?)";
 ...
 */
	...
	currentComponent="tFixedFlowInput_1";
	    ...
	    for (int i_tFixedFlowInput_1 = 0 ; i_tFixedFlowInput_1 < 1 ; i_tFixedFlowInput_1++) {
    	            		...
    	            		row1.jobname = context.jobName;
    	            	        	            	
    	            		row1.lastrun = TalendDate.getCurrentDate();
    	            	        	            	
    	            		row1.result = context.result;
 ...
 */
	...
	currentComponent="tPostgresqlOutput_1";
        ...
        whetherReject_tPostgresqlOutput_1 = false;
                    if(row1.jobname == null) {
pstmt_tPostgresqlOutput_1.setNull(1, java.sql.Types.VARCHAR);
} else {pstmt_tPostgresqlOutput_1.setString(1, row1.jobname);
}

                    if(row1.lastrun != null) {
pstmt_tPostgresqlOutput_1.setTimestamp(2, new java.sql.Timestamp(row1.lastrun.getTime()));
} else {
...
} else {pstmt_tPostgresqlOutput_1.setBoolean(3, row1.result);
}
 ...
 */
    ...
    public static void main(String[] args){
        final JobLogger JobLoggerClass = new JobLogger();

        int exitCode = JobLoggerClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = JobLogger.class.getClassLoader().getResourceAsStream("brk/joblogger_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = JobLogger.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : JobLogger");
        }
...
/************************************************************************************************
 *     51942 characters generated by Talend Open Studio for Data Integration 
