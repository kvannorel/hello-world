...
package brk.perceel_algemeen_0_1;
...
/**
 * Job: perceel_algemeen Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.2.1.20190620_1446
 * @status 
 */
public class perceel_algemeen implements TalendJob {
...
public Boolean getResult(){
	...
	private final String jobName = "perceel_algemeen";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				perceel_algemeen.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(perceel_algemeen.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
try {
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_1", "", Thread.currentThread().getId() + "", "INFO","","Begonnen met rapport algemeen (17/23)","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_1", 3, "Begonnen met rapport algemeen (17/23)", 42);
	tLogCatcher_1Process(globalMap);
	globalMap.put("tWarn_1_WARN_MESSAGES", "Begonnen met rapport algemeen (17/23)"); 
...
public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
    final static byte[] commonByteArrayLock_BRK_perceel_algemeen = new byte[0];
    static byte[] commonByteArray_BRK_perceel_algemeen = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_BRK_perceel_algemeen.length) {
				if(length < 1024 && commonByteArray_BRK_perceel_algemeen.length == 0) {
   					commonByteArray_BRK_perceel_algemeen = new byte[1024];
				} else {
   					commonByteArray_BRK_perceel_algemeen = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_BRK_perceel_algemeen, 0, length);
			strReturn = new String(commonByteArray_BRK_perceel_algemeen, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_BRK_perceel_algemeen) {
...
query_tPostgresPlusRow_1 = "DROP TABLE IF EXISTS brk_perceel_algemeen;\nCREATE TABLE " + Settings.get("SCHEMA_NAME_LOOKUP", "ldf") + ".brk_perceel_algemeen AS SELECT\n  perceel.kdoreferentie AS link_id,\n  NULLIF(concat_ws(' ', perceel.kdoperceelkadaand"
+"uidanaamadgemwaarde, perceel.kdoperceelkadaanduidsectie, perceel.kdoperceelkadaanduidanaamadgemperceelnummer, perceel.kd"
+"operceelkadaanduidanaamadgemappartsrechtvolgnummer), '') AS perceel,\n  upper(NULLIF(concat_ws(' ', subselect.streetname"
+", subselect.housenumber, subselect.houseletter, subselect.housenumberaddition, subselect.zipcode, subselect.city), '')) "
+"AS adres,\n  adrestype AS type,\n  bagidentificatie AS bagidentificatie,\n  NULLIF(concat_ws(' ', COALESCE(adres.straatn"
+"aam_officieel, straatnaam), adres.huisnummer, adres.huisletter, adres.huisnummertoevoeging, adres.postcode, COALESCE(adr"
+"es.authentiekewoonplaatsnaam, adres.woonplaatsnaam)), '') AS perceeladres,\n\n  perceel.kdoperceelkoopsom AS koopsom,\n "
+" perceel.kdoperceelkoopjaar AS koopjaar,\n  perceel.kdoperceelkadgroottewaarde AS grootte,\n  perceel.kdoperceelsoortgro"
+"ottewaarde AS groottesoort,\n  perceel.kdoperceelakrregister9tekst,\n  geom.geom AS geo\n\nFROM brk_kdoperceel AS percee"
+"l\nLEFT JOIN (\n	SELECT perceelid AS perceelid, 'KAD adres' AS adrestype, kadbinobrnaam AS streetname, kadbinhuinum AS h"
+"ousenumber, kadbinhuilet AS houseletter, kadbinhuinumtoev AS housenumberaddition, kadbinpostcode AS zipcode, kadbinwoonp"
+"laats AS city, null AS bagidentificatie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE kadbinpostcode <> '' OR kadbinobrn"
+"aam <> '' OR kadbinwoonplaats <> ''\n	UNION\n	SELECT perceelid AS perceelid, 'VBO hoofdadres' AS adrestype, gerelateerde"
+"obr AS streetname, vbohuinum AS housenumber, vbohuilet AS houseletter, vbohuinumtoev AS housenumberaddition, vbopostcode"
+" AS zipcode, gerelateerdewoonplaats AS city, vbobagidentificatie AS bagidentificatie\n		FROM brk_kdoperceel_locatiekadob"
+"j\n		WHERE vbopostcode <> '' OR gerelateerdeobr <> '' OR gerelateerdewoonplaats <> ''\n	UNION\n	SELECT perceelid AS perc"
+"eelid, 'VBO nevenadres' AS adrestype, vbonevengerelateerdeobr AS streetname, vbonevenhuinum AS housenumber, vbonevenhuil"
+"et AS houseletter, vbonevenhuinumtoev AS housenumberaddition, vbonevenpostcode AS zipcode, vbonevengerelateerdewoonplaat"
+"s AS city, null AS bagidentificatie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE vbonevenpostcode <> '' OR vbonevengere"
+"lateerdeobr <> '' OR vbonevengerelateerdewoonplaats <> ''\n	UNION\n	SELECT perceelid AS perceelid, 'Standplaats hoofdadr"
+"es' AS adrestype, standplaatsgerelateerdeobr AS streetname, standplaatshuinum AS housenumber, standplaatshuilet AS house"
+"letter, standplaatshuinumtoev AS housenumberaddition, standplaatspostcode AS zipcode, standplaatsgerelateerdewoonplaats "
+"AS city, standplaatsbagidentificatie AS bagidentificatie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE standplaatspostco"
+"de <> '' OR standplaatsgerelateerdeobr <> '' OR standplaatsgerelateerdewoonplaats <> ''\n	UNION\n	SELECT perceelid AS pe"
+"rceelid, 'Standplaats nevenadres' AS adrestype, standplaatsnevengerelateerdeobr AS streetname, standplaatsnevenhuinum AS"
+" housenumber, standplaatsnevenhuilet AS houseletter, standplaatsnevenhuinumtoev AS housenumberaddition, standplaatsneven"
+"postcode AS zipcode, standplaatsnevengerelateerdewoonplaats AS city, null AS bagidentificatie\n		FROM brk_kdoperceel_loc"
+"atiekadobj\n		WHERE standplaatsnevenpostcode <> '' OR standplaatsnevengerelateerdeobr <> '' OR standplaatsnevengerelatee"
+"rdewoonplaats <> ''\n	UNION\n	SELECT perceelid AS perceelid, 'Ligplaats hoofdadres' AS adrestype, ligplaatsgerelateerdeo"
+"br AS streetname, ligplaatshuinum AS housenumber, ligplaatshuilet AS houseletter, ligplaatshuinumtoev AS housenumberaddi"
+"tion, ligplaatspostcode AS zipcode, ligplaatsgerelateerdewoonplaats AS city, ligplaatsbagidentificatie AS bagidentificat"
+"ie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE ligplaatspostcode <> '' OR ligplaatsgerelateerdeobr <> '' OR ligplaatsg"
+"erelateerdewoonplaats <> ''\n	UNION\n	SELECT perceelid AS perceelid, 'Ligplaats nevenadres' AS adrestype, ligplaatsneven"
+"gerelateerdeobr AS streetname, ligplaatsnevenhuinum AS housenumber, ligplaatsnevenhuislet AS houseletter, ligplaatsneven"
+"huinumtoev AS housenumberaddition, ligplaatsnevenpostcode AS zipcode, ligplaatsnevengerelateerdewoonplaats AS city, null"
+" AS bagidentificatie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE ligplaatsnevenpostcode <> '' OR ligplaatsnevengerelat"
+"eerdeobr <> '' OR ligplaatsnevengerelateerdewoonplaats <> ''\n	) AS subselect ON perceel.kdoperceelid = subselect.percee"
+"lid\n\nLEFT JOIN anl_adres adres ON replace(subselect.zipcode, ' ', '') = adres.postcode\n     AND subselect.housenumber"
+" = trim(to_char(adres.huisnummer, '99999'))\n    AND concat_ws(' ',subselect.houseletter) = concat_ws(' ',adres.huislett"
+"er)\n    AND concat_ws(' ',subselect.housenumberaddition) = concat_ws(' ',adres.huisnummertoevoeging)\nLEFT JOIN brk_geo"
+"metry geom ON perceel.kdoperceelid = geom.id;\n\nALTER TABLE brk_perceel_algemeen\n  OWNER TO " + Settings.get("CONNECTION_DEFINITION", "user") + ";\nGRANT ALL ON TABLE brk_perceel_algemeen TO " + Settings.get("CONNECTION_DEFINITION", "user") + ";\n\nCOMMENT ON TABLE brk_perceel_algemeen IS 'Perceel algemeen';\nCOMMENT ON COLUMN brk_perceel_algemeen.perceel IS 'P"
+"erceel-id';\nCOMMENT ON COLUMN brk_perceel_algemeen.adres IS 'Kad adres';\nCOMMENT ON COLUMN brk_perceel_algemeen.percee"
+"ladres IS 'Perceel adres';\nCOMMENT ON COLUMN brk_perceel_algemeen.koopsom IS 'Koopsom';\nCOMMENT ON COLUMN brk_perceel_"
+"algemeen.koopjaar IS 'Koopjaar';\n--COMMENT ON COLUMN brk_perceel_algemeen.aantalobjecten IS 'Koopsom aantal objecten';"
+"\nCOMMENT ON COLUMN brk_perceel_algemeen.kdoperceelakrregister9tekst IS 'Register 9 tekst';\nCOMMENT ON COLUMN brk_percee"
+"l_algemeen.geo IS 'Geometry';";
...
context.result = true;
context.jobName = "brk_perceel_algemeen_job";
 ...
 */
    ...
    public static void main(String[] args){
        final perceel_algemeen perceel_algemeenClass = new perceel_algemeen();

        int exitCode = perceel_algemeenClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = perceel_algemeen.class.getClassLoader().getResourceAsStream("brk/perceel_algemeen_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = perceel_algemeen.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : perceel_algemeen");
        }
...
/************************************************************************************************
 *     81854 characters generated by Talend Open Studio for Data Integration 
