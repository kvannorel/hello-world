...
package brk.perceel_koopsomjaar_0_1;
...
/**
 * Job: perceel_koopsomjaar Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.2.1.20190620_1446
 * @status 
 */
public class perceel_koopsomjaar implements TalendJob {
...
public Boolean getResult(){
	...
	private final String jobName = "perceel_koopsomjaar";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				perceel_koopsomjaar.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(perceel_koopsomjaar.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
try {
	...
	resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_1", "", Thread.currentThread().getId() + "", "INFO","","Begonnen met rapport koopsom/koopjaar (19/23)","", "");
	tLogCatcher_1.addMessage("tWarn", "tWarn_1", 3, "Begonnen met rapport koopsom/koopjaar (19/23)", 42);
	tLogCatcher_1Process(globalMap);
	globalMap.put("tWarn_1_WARN_MESSAGES", "Begonnen met rapport koopsom/koopjaar (19/23)"); 
...
public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
    final static byte[] commonByteArrayLock_BRK_perceel_koopsomjaar = new byte[0];
    static byte[] commonByteArray_BRK_perceel_koopsomjaar = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_BRK_perceel_koopsomjaar.length) {
				if(length < 1024 && commonByteArray_BRK_perceel_koopsomjaar.length == 0) {
   					commonByteArray_BRK_perceel_koopsomjaar = new byte[1024];
				} else {
   					commonByteArray_BRK_perceel_koopsomjaar = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_BRK_perceel_koopsomjaar, 0, length);
			strReturn = new String(commonByteArray_BRK_perceel_koopsomjaar, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_BRK_perceel_koopsomjaar) {
...
query_tPostgresqlRow_1 = "DROP TABLE IF EXISTS brk_perceel_koopsomjaar;\nCREATE TABLE " + Settings.get("SCHEMA_NAME_LOOKUP", "ldf") + ".brk_perceel_koopsomjaar AS SELECT\n  perceel.kdoreferentie AS link_id,\n  NULLIF(concat_ws(' ', perceel.kdoperceelkada"
+"anduidanaamadgemwaarde, perceel.kdoperceelkadaanduidsectie, perceel.kdoperceelkadaanduidanaamadgemperceelnummer, perceel"
+".kdoperceelkadaanduidanaamadgemappartsrechtvolgnummer), '') AS perceel,\n  upper(NULLIF(concat_ws(' ', subselect.streetn"
+"ame, subselect.housenumber, subselect.houseletter, subselect.housenumberaddition, subselect.zipcode, subselect.city), ''"
+")) AS adres,\n  adrestype AS type,\n  bagidentificatie AS bagidentificatie,\n  NULLIF(concat_ws(' ', COALESCE(adres.stra"
+"atnaam_officieel, straatnaam), adres.huisnummer, adres.huisletter, adres.huisnummertoevoeging, adres.postcode, COALESCE("
+"adres.authentiekewoonplaatsnaam, adres.woonplaatsnaam)), '') AS perceeladres,\n\n  cast(perceel.kdoperceelkoopsom AS int"
+"eger) AS koopsom,\n  cast (perceel.kdoperceelkoopjaar AS integer) AS koopjaar,\n  perceel.kdoperceelkadgroottewaarde AS "
+"grootte,\n  perceel.kdoperceelsoortgroottewaarde AS groottesoort,\n  perceel.kdoperceelakrregister9tekst,\n  geom.geom A"
+"S geo\n\nFROM brk_kdoperceel AS perceel\nLEFT JOIN (\n	SELECT perceelid AS perceelid, 'KAD adres' AS adrestype, kadbinob"
+"rnaam AS streetname, kadbinhuinum AS housenumber, kadbinhuilet AS houseletter, kadbinhuinumtoev AS housenumberaddition, "
+"kadbinpostcode AS zipcode, kadbinwoonplaats AS city, null AS bagidentificatie\n		FROM brk_kdoperceel_locatiekadobj\n		WH"
+"ERE kadbinpostcode <> '' OR kadbinobrnaam <> '' OR kadbinwoonplaats <> ''\n	UNION\n	SELECT perceelid AS perceelid, 'VBO "
+"hoofdadres' AS adrestype, gerelateerdeobr AS streetname, vbohuinum AS housenumber, vbohuilet AS houseletter, vbohuinumto"
+"ev AS housenumberaddition, vbopostcode AS zipcode, gerelateerdewoonplaats AS city, vbobagidentificatie AS bagidentificat"
+"ie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE vbopostcode <> '' OR gerelateerdeobr <> '' OR gerelateerdewoonplaats <>"
+" ''\n	UNION\n	SELECT perceelid AS perceelid, 'VBO nevenadres' AS adrestype, vbonevengerelateerdeobr AS streetname, vbone"
+"venhuinum AS housenumber, vbonevenhuilet AS houseletter, vbonevenhuinumtoev AS housenumberaddition, vbonevenpostcode AS "
+"zipcode, vbonevengerelateerdewoonplaats AS city, null AS bagidentificatie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE "
+"vbonevenpostcode <> '' OR vbonevengerelateerdeobr <> '' OR vbonevengerelateerdewoonplaats <> ''\n	UNION\n	SELECT perceel"
+"id AS perceelid, 'Standplaats hoofdadres' AS adrestype, standplaatsgerelateerdeobr AS streetname, standplaatshuinum AS h"
+"ousenumber, standplaatshuilet AS houseletter, standplaatshuinumtoev AS housenumberaddition, standplaatspostcode AS zipco"
+"de, standplaatsgerelateerdewoonplaats AS city, standplaatsbagidentificatie AS bagidentificatie\n		FROM brk_kdoperceel_lo"
+"catiekadobj\n		WHERE standplaatspostcode <> '' OR standplaatsgerelateerdeobr <> '' OR standplaatsgerelateerdewoonplaats "
+"<> ''\n	UNION\n	SELECT perceelid AS perceelid, 'Standplaats nevenadres' AS adrestype, standplaatsnevengerelateerdeobr AS"
+" streetname, standplaatsnevenhuinum AS housenumber, standplaatsnevenhuilet AS houseletter, standplaatsnevenhuinumtoev AS"
+" housenumberaddition, standplaatsnevenpostcode AS zipcode, standplaatsnevengerelateerdewoonplaats AS city, null AS bagid"
+"entificatie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE standplaatsnevenpostcode <> '' OR standplaatsnevengerelateerde"
+"obr <> '' OR standplaatsnevengerelateerdewoonplaats <> ''\n	UNION\n	SELECT perceelid AS perceelid, 'Ligplaats hoofdadres"
+"' AS adrestype, ligplaatsgerelateerdeobr AS streetname, ligplaatshuinum AS housenumber, ligplaatshuilet AS houseletter, "
+"ligplaatshuinumtoev AS housenumberaddition, ligplaatspostcode AS zipcode, ligplaatsgerelateerdewoonplaats AS city, ligpl"
+"aatsbagidentificatie AS bagidentificatie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE ligplaatspostcode <> '' OR ligpla"
+"atsgerelateerdeobr <> '' OR ligplaatsgerelateerdewoonplaats <> ''\n	UNION\n	SELECT perceelid AS perceelid, 'Ligplaats ne"
+"venadres' AS adrestype, ligplaatsnevengerelateerdeobr AS streetname, ligplaatsnevenhuinum AS housenumber, ligplaatsneven"
+"huislet AS houseletter, ligplaatsnevenhuinumtoev AS housenumberaddition, ligplaatsnevenpostcode AS zipcode, ligplaatsnev"
+"engerelateerdewoonplaats AS city, null AS bagidentificatie\n		FROM brk_kdoperceel_locatiekadobj\n		WHERE ligplaatsnevenp"
+"ostcode <> '' OR ligplaatsnevengerelateerdeobr <> '' OR ligplaatsnevengerelateerdewoonplaats <> ''\n	) AS subselect ON p"
+"erceel.kdoperceelid = subselect.perceelid\n	\nLEFT JOIN anl_adres adres ON replace(subselect.zipcode, ' ', '') = adres.p"
+"ostcode\n     AND subselect.housenumber = trim(to_char(adres.huisnummer, '99999'))\n    AND concat_ws(' ',subselect.hous"
+"eletter) = concat_ws(' ',adres.huisletter)\n    AND concat_ws(' ',subselect.housenumberaddition) = concat_ws(' ',adres.h"
+"uisnummertoevoeging)\n\nLEFT JOIN brk_geometry geom ON geom.id = perceel.kdoperceelid;\n\nALTER TABLE brk_perceel_koopso"
+"mjaar\n  OWNER TO " + Settings.get("CONNECTION_DEFINITION", "user") + ";\nGRANT ALL ON TABLE brk_perceel_algemeen TO " + Settings.get("CONNECTION_DEFINITION", "user") + ";\n\nCOMMENT ON TABLE brk_perceel_koopsomjaar IS 'Perceel koopsom koopjaar';\nCOMMENT ON COLUMN brk_perceel_koopsomjaar"
+".perceel IS 'Perceel-id';\nCOMMENT ON COLUMN brk_perceel_koopsomjaar.adres IS 'Kad adres';\nCOMMENT ON COLUMN brk_percee"
+"l_koopsomjaar.perceeladres IS 'Perceel adres';\nCOMMENT ON COLUMN brk_perceel_koopsomjaar.koopsom IS 'Koopsom';\nCOMMENT"
+" ON COLUMN brk_perceel_koopsomjaar.koopjaar IS 'Koopjaar';\nCOMMENT ON COLUMN brk_perceel_koopsomjaar.kdoperceelakrregis"
+"ter9tekst IS 'Register 9 tekst';\nCOMMENT ON COLUMN brk_perceel_koopsomjaar.geo IS 'Geo object';";
...
context.jobName = "brk_perceel_koopsomjaar_job";
 ...
 */
    ...
    public static void main(String[] args){
        final perceel_koopsomjaar perceel_koopsomjaarClass = new perceel_koopsomjaar();

        int exitCode = perceel_koopsomjaarClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = perceel_koopsomjaar.class.getClassLoader().getResourceAsStream("brk/perceel_koopsomjaar_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = perceel_koopsomjaar.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : perceel_koopsomjaar");
        }
...
/************************************************************************************************
 *     82731 characters generated by Talend Open Studio for Data Integration 
